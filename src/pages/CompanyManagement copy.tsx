...
export default function CompanyManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!editingCompany && (!form.admin_prenom || !form.admin_nom || !form.admin_email)) {
    ...
    await execute(async () => {
      try {
        if (editingCompany) {
          await superAdminService.updateCompany(editingCompany.id, form)
        } else {
          // Créer l'entreprise avec les informations de l'administrateur
          const companyData = {
            ...form,
            admin: {
              nom: form.admin_nom,
              ...
              password: form.admin_password || 'admin123'
            }
          }
  ...
  const exportCompanies = () => {
    ...
    const headers = [
      ...
      'Secteur', 'Site Web', 'Date Création'
    ]
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          {activeTab === 'admin' && !editingCompany && (
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700 mt-1">
                      Un compte administrateur sera automatiquement créé pour cette entreprise.
                      Cet utilisateur aura tous les droits d'administration sur l'entreprise.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm(prev => ({ ...prev, admin_prenom: e.target.value }))}
                    className="input-field"
                    placeholder="Jean"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm(prev => ({ ...prev, admin_nom: e.target.value }))}
                    className="input-field"
                    placeholder="Dupont"
                  />
                </div>
          ...
          <div className="flex justify-between space-x-3 mt-6 pt-6 border-t">
            <div className="flex space-x-2">
              ...
              {activeTab === 'general' && !editingCompany && (
                <button
                  type="button"
                  onClick={() => setActiveTab('admin')}
                  className="btn-secondary"
